...
class LegFootJTCPublisher(Node):

    def __init__(self):
        super().__init__('leg_foot_jtc_publisher_node')
        ...
        if self.leg == 'right':
            ...
            self.get_logger().info("Controlled Joints: RL_Revolute87")
            self.joints = ["RL_Revolute87"]
            # Topic to publish to
            self.publish_topic = "/right_leg_foot_controller/joint_trajectory"
        elif self.leg == 'left':
            self.get_logger().info("Leg: Left Leg")
            self.get_logger().info("Controlled Joints: LL_Revolute87")
            self.joints = ["LL_Revolute87"]
            # Topic to publish to
            self.publish_topic = "/left_leg_foot_controller/joint_trajectory"
        else:
