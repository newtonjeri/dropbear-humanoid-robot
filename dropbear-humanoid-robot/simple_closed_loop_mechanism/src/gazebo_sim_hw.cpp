...
#include "simple_closed_loop_mechanism/gazebo_sim_hw.hpp"
...
bool GazeboSimHardware::initSim(
...
bool GazeboSimHardware::extractPID(
...
bool GazeboSimHardware::extractPIDFromParameters(
...
void GazeboSimHardware::registerJoints(
  const hardware_interface::HardwareInfo & hardware_info,
  gazebo::physics::ModelPtr parent_model)
{
...
void GazeboSimHardware::registerSensors(
  const hardware_interface::HardwareInfo & hardware_info,
  gazebo::physics::ModelPtr parent_model)
{
...
CallbackReturn
GazeboSimHardware::on_init(const hardware_interface::HardwareInfo & system_info)
{
...
GazeboSimHardware::export_state_interfaces()
{
  return std::move(this->dataPtr->state_interfaces_);
}

std::vector<hardware_interface::CommandInterface>
GazeboSimHardware::export_command_interfaces()
{
  return std::move(this->dataPtr->command_interfaces_);
}

CallbackReturn GazeboSimHardware::on_activate(const rclcpp_lifecycle::State & previous_state)
{
  return CallbackReturn::SUCCESS;
}

CallbackReturn GazeboSimHardware::on_deactivate(const rclcpp_lifecycle::State & previous_state)
{
  return CallbackReturn::SUCCESS;
}

hardware_interface::return_type
GazeboSimHardware::perform_command_mode_switch(
...
hardware_interface::return_type GazeboSimHardware::read(
...
hardware_interface::return_type GazeboSimHardware::write(
  const rclcpp::Time & time,
  const rclcpp::Duration & period)
{
...
PLUGINLIB_EXPORT_CLASS(
  gazebo_ros2_control::GazeboSimHardware, gazebo_ros2_control::GazeboSystemInterface)